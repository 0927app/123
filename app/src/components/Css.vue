<template>
  <div class="container">
    ...
    <div class="jumbotron jumbotron-fluid">
      ...
      <div class="container">
        <h1 class="display-5">
          <span class="badge badge-dark mr-2">{{ css[progress].id }}</span
          >{{ css[progress].subject }}
        </h1>
        ...
        <div v-if="this.progress < this.size">
          <div
            ...
            v-for="(i, index) in css[progress].item"
            :key="index"
          >
        ...
        <!-- 多选题 -->
        <div v-else-if="this.progress < this.css.length">
          <div
            class="form-check my-3"
            v-for="(i, index) in css[progress].item"
            :key="index"
        ...
        <table class="table">
          ...
          <tbody class="text-center">
            <tr v-for="(i, index) in answers" :key="index">
              ...
              <td>
                <span class="badge badge-primary">{{ css[index].answer }}</span>
              </td>
              <td v-if="i == css[index].answer">正确</td>
              <td v-else>错误</td>
            </tr>
            <tr>
  ...
  methods: {
    ...
    down() {
      if (this.progress < this.size) {
        ...
        //清空
        this.answe = [];
      } else if (this.progress < this.css.length) {
        this.progress++;
        let s = "";
    ...
    submitexam() {
      this.down();
      if ((this.progress = this.css.length - 1)) {
        this.submit = true;
      }
      for (let i = 0; i < this.answers.length; i++) {
        if (this.answers[i] == this.css[i].answer) {
          this.sum += 10;
        }
      }
